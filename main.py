...
# ၂။ Session State (History အတွက် ၁၀ ခုပဲ ကန့်သတ်မယ်)
if 'recent_results' not in st.session_state:
    st.session_state.recent_results = [] # ၁၀ ခုစာပဲ သိမ်းရန်
...
# --- UI ---
st.title("🎯 TRX AI Smart Predictor (Status Bar)")

# ၃။ Win/Loss Status Bar (၁၀ ကွက်)
...
if st.session_state.active_pattern:
    ...
    with col_a:
        st.info(f"Selected Pattern: **{st.session_state.active_pattern}**")
        if pred:
    ...
    with col_b:
        ...
        def record_result(actual):
            ...
            if st.session_state.current_prediction:
                ...
                # ၁၀ ခုပြည့်ရင် Reset လုပ်မယ်
                if len(st.session_state.recent_results) > 10:
                    st.session_state.recent_results = [status]
            
            save_new_data(actual) # CSV ထဲသိမ်းမယ်
            st.session_state.active_pattern = None 
            st.session_state.current_prediction = None
            st.rerun()
...
# Footer Information
with st.sidebar:
    st.write("### ⚙️ Statistics")
    if st.session_state.recent_results:
        w_count = st.session_state.recent_results.count("W")
        l_count = st.session_state.recent_results.count("L")
        st.write(f"Wins in this round: **{w_count}**")
        st.write(f"Losses in this round: **{l_count}**")
        if st.button("Reset Bar Manually"):
